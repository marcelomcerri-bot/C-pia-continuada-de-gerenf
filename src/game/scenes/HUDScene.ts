...
export class HUDScene extends Phaser.Scene {
  ...
  public toggleMissionOverlay(state: GameState) {
    ...
    const W = this.scale.width, H = this.scale.height;
    const panelW = Math.min(800, W - 32), panelH = Math.min(500, H - 40);
    const c = this.add.container(W / 2, H / 2).setDepth(300);

    const bg = this.add.graphics();
    // Dark medical inset backdrop
    bg.fillStyle(0x050b14, 0.98);
    bg.fillRoundedRect(-panelW / 2, -panelH / 2, panelW, panelH, 12);
    // Double high-tech glowing border
    bg.lineStyle(1.8, 0x1abc9c, 0.85);
    bg.strokeRoundedRect(-panelW / 2, -panelH / 2, panelW, panelH, 12);
    bg.lineStyle(1, 0x1abc9c, 0.2);
    bg.strokeRoundedRect(-panelW / 2 - 3, -panelH / 2 - 3, panelW + 6, panelH + 6, 14);

    // Subtle technical grid background lines inside panel
    bg.lineStyle(1, 0x1abc9c, 0.05);
    for (let gx = -panelW / 2 + 30; gx < panelW / 2; gx += 40) {
      bg.lineBetween(gx, -panelH / 2 + 10, gx, panelH / 2 - 40);
    }

    // Title & Subtitle bar
    const title = this.add.text(0, -panelH / 2 + 22, 'DIRETRIZES E MISSÕES DE GERÊNCIA', {
      fontFamily: "'Press Start 2P', monospace", fontSize: '13px', color: '#1abc9c',
    }).setOrigin(0.5);

    const lvInfo = getLevelInfo(state.prestige);
    const pName = (state.playerProfile?.name || 'Enf. Alex').toUpperCase();
    const careerTxt = this.add.text(0, -panelH / 2 + 44,
      `${pName}  ·  ${lvInfo.title.toUpperCase()}  ·  ⭐ ${state.prestige} PTS`, {
        fontFamily: "'Press Start 2P', monospace", fontSize: '9px', color: '#f1c40f',
      }).setOrigin(0.5);

    // Close button
    const closeBtn = this.add.text(-panelW / 2 + 20, -panelH / 2 + 16, '✕', {
      fontFamily: "'Press Start 2P', monospace", fontSize: '14px', color: '#e74c3c',
    }).setInteractive({ cursor: 'pointer', padding: 10 })
      .on('pointerdown', () => { playSound('click'); c.destroy(); this.missionOverlay = null; });

    // Hover scales
    closeBtn.on('pointerover', () => closeBtn.setColor('#ff7675'));
    ...
    for (const cat of leftCats) {
      ...
      // Draw category title
      const catLabel = this.add.text(-panelW / 2 + 25, ly, `■ ${cat.toUpperCase()}`, {
        fontFamily: "'Press Start 2P', monospace", fontSize: '9px', color: '#1abc9c',
      });
      items.push(catLabel);
      ly += 16;
      ...
      for (const m of catMissions) {
        ...
        const col = done ? '#2ecc71' : active ? '#f1c40f' : locked ? '#576574' : '#bdc3c7';

        const line = this.add.text(-panelW / 2 + 32, ly, `${icon} ${m.title}`, {
          fontFamily: "'VT323', monospace", fontSize: '20px', color: col,
        });
        items.push(line);
        ly += 20;
      }
    ...
    for (const cat of rightCats) {
      ...
      // Draw category title
      const catLabel = this.add.text(15, ry, `■ ${cat.toUpperCase()}`, {
        fontFamily: "'Press Start 2P', monospace", fontSize: '9px', color: '#1abc9c',
      });
      items.push(catLabel);
      ry += 16;
      ...
      for (const m of catMissions) {
        ...
        const line = this.add.text(22, ry, `${icon} ${m.title}`, {
          fontFamily: "'VT323', monospace", fontSize: '20px', color: col,
        });
        items.push(line);
        ry += 20;
    ...
    const doneCount = state.completedMissions.length;
    const totalCount = MISSIONS.length;
    const pct = totalCount > 0 ? ((doneCount / totalCount) * 100) | 0 : 0;

    // Bottom telemetry stats
    const progText = this.add.text(-panelW / 2 + 25, panelH / 2 - 32,
      `PROGRESSO GERAL: ${doneCount}/${totalCount} (${pct}%)`, {
        fontFamily: "'Press Start 2P', monospace", fontSize: '9px', color: '#bdc3c7',
      }).setOrigin(0, 0.5);

    // Visual Progress Bar
    const barX = -panelW / 2 + 25;
    const barY = panelH / 2 - 20;
    const barWidth = 320;
    const barHeight = 8;
    
    // Draw Progress Bar track
    bg.fillStyle(0x0e1626, 1);
    bg.fillRoundedRect(barX, barY, barWidth, barHeight, 4);
    bg.lineStyle(1, 0x1abc9c, 0.3);
    bg.strokeRoundedRect(barX, barY, barWidth, barHeight, 4);
    
    // Draw Progress Bar fill
    if (pct > 0) {
      bg.fillStyle(0x2ecc71, 1);
      bg.fillRoundedRect(barX, barY, (barWidth * pct) / 100, barHeight, 4);
    }

    // Stress metric display
    const stressVal = Math.floor(state.stress || 0);
    const stressCol = stressVal > 60 ? '#e74c3c' : stressVal > 30 ? '#f39c12' : '#2ecc71';
    
    const stressText = this.add.text(panelW / 2 - 25, panelH / 2 - 26,
      `ESTRESSE OCUPACIONAL: ${stressVal}% ⚡`, {
        fontFamily: "'Press Start 2P', monospace", fontSize: '9px', color: stressCol,
      }).setOrigin(1, 0.5);

    c.add([bg, title, careerTxt, closeBtn, ...items, progText, stressText]);
    c.setScale(0.95).setAlpha(0);
